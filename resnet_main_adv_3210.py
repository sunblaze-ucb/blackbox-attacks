...
import numpy as np
import resnet_model_reusable_3210
...
def train(hps):
  ...
  noleak_model = resnet_model_reusable_3210.ResNet(hps, images_scaled, None, 'eval')
  ...
  adv_model = resnet_model_reusable_3210.ResNet(hps, images_scaled, noleak_labels, 'eval', reuse_variables=True)
  ...
  model = resnet_model_reusable_3210.ResNet(grand_hps, grand_images_scaled, grand_labels, FLAGS.mode, reuse_variables=True)
  model.build_graph()
  ...
  stop_hook = tf.train.StopAtStepHook(last_step=160000)
...
def evaluate(hps):
  ...
  model = resnet_model_reusable_3210.ResNet(hps, images, labels, FLAGS.mode)
  model.build_graph()
...
def main(_):
  ...
  hps = resnet_model_reusable_3210.HParams(batch_size=batch_size,
