...
def train(hps):
  ...
  model = resnet_model_reusable.ResNet(grand_hps, grand_images_scaled, grand_labels, FLAGS.mode, reuse_variables=True)
  model.build_graph()
...
def evaluate(hps):
  ...
  model = resnet_model_reusable.ResNet(hps, images, labels, FLAGS.mode)
  model.build_graph()
